...
void ft_reverse_rotate_a(t_base *base, char flag)
{
	if (!(base->stack_A && base->stack_A->next))
		return ;
	ft_lstadd_front(&base->stack_A, ft_pop_back(&base->stack_A));
	if (flag)
		write(1, "rra\n", 4);
}
...
void ft_reverse_rotate_b(t_base *base, char flag)
{
	if (!(base->stack_B && base->stack_B->next))
		return ;
	ft_lstadd_front(&base->stack_B, ft_pop_back(&base->stack_B));
	if (flag)
		write(1, "rrb\n", 4);
}
...
void ft_reverse_rotate_ab(t_base *base, char flag)
{
	...
	ft_reverse_rotate_a(base, flag && !(exist_A && exist_B));
	ft_reverse_rotate_b(base, flag && !(exist_A && exist_B));
